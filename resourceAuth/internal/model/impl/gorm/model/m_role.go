...
var _ model2.IRole = (*Role)(nil)

// RoleSet 注入Role
var RoleSet = wire.NewSet(wire.Struct(new(Role), "*"), wire.Bind(new(model2.IRole), new(*Role)))
...
func (a *Role) Query(ctx context.Context, params schema.RoleQueryParam, opts ...schema.RoleQueryOptions) (*schema.RoleQueryResult, error) {
	...
	if v := params.AccountKey; v != "" {
		subQuery := entity2.GetAccountRoleDB(ctx, a.DB).
			Where("accountKey = ?", v).
