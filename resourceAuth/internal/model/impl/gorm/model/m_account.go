...
var _ model2.IAccount = (*Account)(nil)

// AccountSet 注入Account
var AccountSet = wire.NewSet(wire.Struct(new(Account), "*"), wire.Bind(new(model2.IAccount), new(*Account)))
...
func (a *Account) Query(ctx context.Context, params schema.AccountQueryParam, opts ...schema.AccountQueryOptions) (*schema.AccountQueryResult, error) {
	...
	db := entity2.GetAccountDB(ctx, a.DB)
...
func (a *Account) Create(ctx context.Context, item schema.Account) error {
	sAccount := entity2.SchemaAccount(item)
	result := entity2.GetAccountDB(ctx, a.DB).Create(sAccount.ToNewAccount())
