...
type Account struct {
	...
	TransModel         model2.ITrans
	AccountModel       model2.IAccount
	AccountSystemModel model2.IAccountSystem
	AccountRoleModel   model2.IAccountRole
	RoleModel          model2.IRole
}

var _ bll2.IAccount = (*Account)(nil)

// AccountSet 注入Account
var AccountSet = wire.NewSet(wire.Struct(new(Account), "*"), wire.Bind(new(bll2.IAccount), new(*Account)))
...
func (a *Account) Create(ctx context.Context, item schema.AccountCreateParam) (string, error) {
	...
	account.AccountKey = iutil2.NewID()
