...
type (
	traceIDKey struct{}
	accountKeyKey  struct{}
)

func CreateTraceIDContext(ctx context.Context, traceID string) context.Context {
...
// NewAccountKeyContext 创建账号标识上下文
func NewAccountKeyContext(ctx context.Context, accountKey string) context.Context {
	return context.WithValue(ctx, accountKeyKey{}, accountKey)
}

// FromAccountKeyContext 从上下文中获取账号标识
func FromUserIDContext(ctx context.Context) string {
	v := ctx.Value(accountKeyKey{})
	if v != nil {
		if s, ok := v.(string); ok {
			return s
		}
