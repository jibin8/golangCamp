/*
 * 使用https://github.com/parnurzeal/gorequest 作为蓝本
 * 主要封装了如下功能：
 *    1. 程序为非200时，打印相应的日志
 *    2. POST的Send支持map[string]interface
 *    3. 修改TagetType的默认值为form(原为json)
 *
 */
...
func NewRequest() *SuperAgent {
	cookiejarOptions := cookiejar.Options{
		PublicSuffixList: publicsuffix.List,
	}
	jar, _ := cookiejar.New(&cookiejarOptions)
	s := &SuperAgent{
		TargetType:        "form", // 修改为form
...
// format errors
func (s *SuperAgent) FormatErrors(errs []error) string {
	ret := ""
	sz := len(errs)
	...
	for i := 0; i < sz; i++ {
		...
		if i == sz-1 {
			ret += fmt.Sprintf("%s", errs[i].Error())
		} else {
			ret += fmt.Sprintf("%s,", errs[i].Error())
		}
	}
...
func (s *SuperAgent) Post(targetUrl, forceType string) *SuperAgent {
	s.ClearSuperAgent()
	s.Method = POST
	s.Url = targetUrl
	s.Errors = nil
	if len(forceType) > 0 {
		s.ForceType = forceType
	}
	return s
}
...
func (s *SuperAgent) Sets(headers map[string]string) *SuperAgent {
	s.Header = headers
	return s
}
...
func (s *SuperAgent) SendBody(body string) *SuperAgent {
	s.Body = body
	return s
}
...
func (s *SuperAgent) End(callback ...func(response Response, body string, errs []error)) (Response, string, []error) {
	...
	//dlog.Debugf("[url: %s]", s.Url)
	if errs == nil && resp.StatusCode != 200 {
		dlog2.Errorf("http request error [code: %d][content:%s]", resp.StatusCode, body)
	}
